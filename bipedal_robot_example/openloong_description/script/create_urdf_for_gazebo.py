...
"""
Gazebo would merge all the links connected by fixed joints into one link,
and usally the imu link is connected to the root link by a fixed joint.
But we would use it in the gazebo hardware interface, so: 
If there is a link with name containing 'imu':
    If the link has no inertial tag, add it
    add the disableFixedJointLumping tag in the gazebo tag
If there not a link with name containing 'imu':
    manully add a imu link fixed to the root link
    
The disableFixedJointLumping tag is like:
...
# if there is no imu link, add it
if imu_link is None:
    print('No link with name containing imu found, adding imu link manually')
    # add the link tag
    new_imu_link_name = input('Enter the name of the imu link: ')
    new_imu_link = ET.Element('link')
    new_imu_link.set('name', new_imu_link_name)
    inertial = ET.SubElement(new_imu_link, 'inertial')
    ...
    root.append(new_imu_link)
    # add the fixed joint tag
    new_imu_joint_name = input('Enter the name of the imu joint: ')
    new_imu_joint = ET.Element('joint')
    new_imu_joint.set('name', new_imu_joint_name)
    new_imu_joint.set('type', 'fixed')
    origin = ET.SubElement(new_imu_joint, 'origin')
    origin.set('xyz', '0 0 0')
    origin.set('rpy', '0 0 0')
    parent = ET.SubElement(new_imu_joint, 'parent')
